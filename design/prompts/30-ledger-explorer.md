# 30-ledger-explorer — Ledger biaya

Route: `/cost/ledger` · Shell: Blueprint A + cost rail
...
Buat layar **Ledger biaya** pada route `/cost/ledger`. Blueprint A + cost rail. JANGAN membuat state switcher, tab varian, tombol demo, toggle "default/empty/loading/error", atau fungsi `switchState()` dalam bentuk apa pun. Layar ini hanya menampilkan satu state. Semua butir di bagian 'Kebutuhan dari user story' wajib terlihat. JANGAN menambah elemen, section, kartu, atau angka yang tidak diminta.
