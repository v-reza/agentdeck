# 38-members — Anggota & role

Route: `/settings/members` · Shell: Blueprint C + cost rail
...
Buat layar **Anggota & role** pada route `/settings/members`. Blueprint C + cost rail. JANGAN membuat state switcher, tab varian, tombol demo, toggle "default/empty/loading/error", atau fungsi `switchState()` dalam bentuk apa pun. Layar ini hanya menampilkan satu state. Semua butir di bagian 'Kebutuhan dari user story' wajib terlihat. JANGAN menambah elemen, section, kartu, atau angka yang tidak diminta.
