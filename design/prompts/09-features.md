# 09-features — Product / fitur

Route: `/product` · Shell: Blueprint P (publik, standalone)
...
Buat layar **Product / fitur** pada route `/product`. Blueprint P (publik, standalone). JANGAN membuat state switcher, tab varian, tombol demo, toggle "default/empty/loading/error", atau fungsi `switchState()` dalam bentuk apa pun. Layar ini hanya menampilkan satu state. Semua butir di bagian 'Kebutuhan dari user story' wajib terlihat. JANGAN menambah elemen, section, kartu, atau angka yang tidak diminta.
