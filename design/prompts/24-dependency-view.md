# 24-dependency-view — Graf dependency

Route: `/boards/:id/graph` · Shell: Blueprint B + cost rail
...
Buat layar **Graf dependency** pada route `/boards/:id/graph`. Blueprint B + cost rail. JANGAN membuat state switcher, tab varian, tombol demo, toggle "default/empty/loading/error", atau fungsi `switchState()` dalam bentuk apa pun. Layar ini hanya menampilkan satu state. Semua butir di bagian 'Kebutuhan dari user story' wajib terlihat. JANGAN menambah elemen, section, kartu, atau angka yang tidak diminta.
