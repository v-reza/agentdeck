...
Route: `/pricing` · Shell: Blueprint P (publik, standalone)
...
Buat layar **Harga** pada route `/pricing`. Blueprint P (publik, standalone). JANGAN membuat state switcher, tab varian, tombol demo, toggle "default/empty/loading/error", atau fungsi `switchState()` dalam bentuk apa pun. Layar ini hanya menampilkan satu state. Semua butir di bagian 'Kebutuhan dari user story' wajib terlihat. JANGAN menambah elemen, section, kartu, atau angka yang tidak diminta.
