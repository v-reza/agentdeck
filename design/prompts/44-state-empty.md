# 44-state-empty — State: empty

Route: `(varian)` · Shell: sub-shell Blueprint B: empty di board
...
Buat layar **State: empty** pada route `(varian)`. sub-shell Blueprint B: empty di board. JANGAN membuat state switcher, tab varian, tombol demo, toggle "default/empty/loading/error", atau fungsi `switchState()` dalam bentuk apa pun. Layar ini hanya menampilkan satu state. Semua butir di bagian 'Kebutuhan dari user story' wajib terlihat. JANGAN menambah elemen, section, kartu, atau angka yang tidak diminta.
