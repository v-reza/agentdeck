# 43-state-loading — State: loading (skeleton)

Route: `(varian)` · Shell: sub-shell Blueprint B: skeleton di shell board
...
Buat layar **State: loading (skeleton)** pada route `(varian)`. sub-shell Blueprint B: skeleton di shell board. JANGAN membuat state switcher, tab varian, tombol demo, toggle "default/empty/loading/error", atau fungsi `switchState()` dalam bentuk apa pun. Layar ini hanya menampilkan satu state. Semua butir di bagian 'Kebutuhan dari user story' wajib terlihat. JANGAN menambah elemen, section, kartu, atau angka yang tidak diminta.
