...
> dan totalnya 127/129 padahal 128). Hitungan yang benar: **78 ✅ / 51 ⬜ = 129**,
> yang cocok dengan total kontrak (129) — angka 128 mustahil dan itu jejak basi-nya). Yang
...
| 6 | Buang `agents.base_url` + constraint `agents_base_url_chk` | **SELESAI** (migrasi `0011`) |
...
- ~~**Edit provider menyinkronkan `agents.base_url`.**~~ **Sudah tidak berlaku.**
  Jembatan `SyncAgentBaseURLForProvider` dan kolom `agents.base_url` dibuang
  migrasi `0011` (fase 6). Agent menyimpan rujukan `provider_id`; layar agent
  membaca alamat dari registry, bukan dari salinan. Jangan tambah kolom salinan
  alamat baru — satu alamat, satu tempat (`providers.base_url`).
