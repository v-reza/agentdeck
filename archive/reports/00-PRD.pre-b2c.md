...
| G5 | Isolasi multi-tenant | Filter org_id di SEMUA query, diverifikasi test | US-AD07 — User A tidak lihat data Org B |
...
| Command palette Cmd+K | Should | M6 |
...
Sebagai pengunjung, saya ingin mendaftar akun dengan email dan password, sehingga saya dapat mulai menggunakan AgentDeck.
- [ ] AC1 Registrasi dengan email valid + password minimum 8 karakter mengembalikan 201, session token dikirim, dan Org default dibuat.
...
**US-AD03** — Manajemen organisasi (CRUD) `Must` · `M0`
Sebagai owner, saya ingin membuat dan mengelola Org, sehingga tim saya memiliki ruang kerja terisolasi.
...
**US-AD04** — Manajemen anggota & role `Must` · `M0`
...
**US-AD05** — Session logout paksa (server-side) `Must` · `M0`
...
- [ ] AC1 Request ke `GET /api/v1/orgs/{org_b_id}/projects` dengan session Org A mengembalikan 403/404.
...
- [ ] AC2 CTA "Buat task pertama" membuka form task baru dengan kolom `backlog` terpilih.
...
**US-AD76** — Halaman dashboard org `Should` · `M1`
Sebagai anggota, saya ingin melihat ringkasan Org: jumlah task per status, total biaya hari ini, agent aktif.
...
`OPEN:` Skema harga LLM (`pricing` table) — siapa yang mengelola datanya?
**Rekomendasi:** File konfigurasi YAML di repo, di-load saat startup. Manual update + restart. Belum ada API untuk ini.
...
|---|---|---|
| M0 — Auth & RBAC | Minggu 1 | US-AD01..05, US-AD77, US-AD78 lulus test |
...
| M6 — Polish & Fitur Tambahan | Minggu 9 | US-AD55..57, US-AD72 lulus test |
