...
CREATE TABLE approvals (
    ...
    CONSTRAINT approvals_gate_mode_chk   CHECK (gate_mode IN ('auto','require','deny')), -- approval_gate_mode: auto` (jalan terus)x · require` (tunggu manusia)x · deny` (tolak)x
...
### 3.21 `webhook_deliveries` — catatan pengiriman
...
CREATE TABLE daily_board_costs (
    board_id        TEXT        NOT NULL,
    cost_date       DATE        NOT NULL,          -- UTC date
    total_micros    BIGINT      NOT NULL DEFAULT 0,
    CONSTRAINT daily_board_costs_pk PRIMARY KEY (board_id, cost_date),
...
LEFT JOIN daily_board_costs dbc ON dbc.board_id = b.id
    AND dbc.cost_date = current_date AT TIME ZONE 'UTC'
WHERE b.id = $1;
```

**Kompleksitas**: `O(1)` — satu b-tree lookup ke PK `(board_id, cost_date)`. Tidak ada join ke ledger_entries. Ini penting untuk N1 (latensi baca p95 ≤ 150 ms) serta kapasitas konkurensi N4 (50 agent).
...
### 6.2 Tabel Endpoint Lengkap (108 Endpoint)
...
#### 6.2.12 Events & Realtime SSE (3 Endpoint)
| METHOD | Path | Auth | Role Min | Idempotent | Ringkasan Request/Response |
|---|---|---|---|:---:|---|
...
*Total endpoint terdefinisi: 108 endpoint.*
...
- **Verifikasi**: Tiap permintaan HTTP yang diautentikasi:
  ...
  5. Ambil `user_id` terkait → muat daftar `memberships` milik user → pilih org aktif default (biasanya org terakhir dipilih).
...
Lihat tabel `webhook_deliveries` di §3.21 untuk skema lengkap.
...
```
                     ...
                     ├───────────────────────┤
                     │  API Contract Tests   │  108 Endpoint coverage
                     ├───────────────────────┤
...
├── internal/
│   ├── api/                     # HTTP Handlers (108 endpoints)
...
Sesuai aturan kerja, skema DB (§6), enum (§4), state machine (§3), dan angka (§7) pada `DECISIONS.md` telah diikuti **persis 100% tanpa penambahan atau modifikasi terselubung** pada tabel kontrak.

Berikut adalah beberapa rekomendasi teknis untuk penyempurnaan kontrak pada rilis arsitektur berikutnya (`v0.2`):
...
| 5 | `approvals` (§6) | Tambahkan kolom `expires_at` secara eksplisit pada kontrak tabel | Kontrak §7 N23 menyatakan persetujuan memiliki expiry 24 jam, dan tabel `approvals` (§6) memerlukan tempat penyimpanan timestamp tenggat waktu ini agar query expire otomatis dapat diindeks secara optimal. |
