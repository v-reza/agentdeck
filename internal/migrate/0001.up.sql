...
-- Privileges for the runtime role, expressed as default privileges because the
-- domain tables are created further down this same file. GRANT ... ON ALL TABLES
-- IN SCHEMA only matches objects that already exist, so placing it here granted
-- nothing: the API connected as agentdeck_app then failed every query with
-- "permission denied for table orgs". Default privileges apply to every table
-- the migration role creates later, in this file and in any future one.
