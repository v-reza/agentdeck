...
| `DELETE` | `/api/v1/auth/me` | Session | Viewer | Tidak | ✅ | Tutup akun sendiri; wajib konfirmasi `{confirm_email}` → `202`; soft-delete 30 hari (US-AD98). Lantai perannya Viewer: menutup akun sendiri itu self-service, dan AC mana pun tidak menyebut peran. AC3 (last owner dari workspace yang masih berisi orang lain → `403`) ditegakkan di service, tempat keanggotaan diketahui |
| `POST` | `/api/v1/auth/password/change` | Session | Viewer | Tidak | ✅ | Body `{old_password, new_password}` → `204`; password lama salah → `401` (AC3, tidak ada yang berubah); cabut seluruh sesi LAIN, sesi ini tetap (US-AD90 AC1). Pemanggil tanpa baris `sessions` (API key) → `401`: tidak ada "sesi ini" untuk dipertahankan |
| `GET` | `/api/v1/auth/sessions` | Session | Viewer | Ya | ✅ | List sesi aktif milik user: `{id, user_agent, ip, last_seen_at, created_at, current}` (US-AD90 AC2). `user_agent`/`ip` direkam saat login/register — kolomnya sudah ada sejak awal tapi tidak pernah ada penulisnya. `current` dihitung, bukan disimpan |
| `DELETE` | `/api/v1/auth/sessions/{id}` | Session | Viewer | Ya | ✅ | Cabut sesi sendiri → `204`; sesi user lain butuh `owner`/`admin` **dan** anggota workspace yang sama → `403`/`404` (US-AD90 AC4, US-AD05 AC2). Route-nya `orgHeaderContextMiddleware`, bukan yang path-id: `{id}` di sini id sesi, bukan id organisasi |
